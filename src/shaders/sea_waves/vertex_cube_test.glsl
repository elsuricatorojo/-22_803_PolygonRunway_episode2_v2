...
void main(){
    ...
    float frecuencia1 = 6.;
    float magnitud1 = 0.05;
    vec2 puntoRotado1 = rotate(vec2(position.x, position.y), 1.);
    float elevation1 = sin(puntoRotado1.y*frecuencia1 + uTime*0.8)*magnitud1;
    elevation1 *= 1.;

    float elevation = elevation1;

    for(float i=1.0; i<=3.0; i++){
        elevation -= abs(cnoise(vec3(modelPosition.xz*0.8*i, uTime*0.6)) * 0.15/i*uBigWavesElevation*4.5);
    }

    if(modelPosition.y > 0.2){
